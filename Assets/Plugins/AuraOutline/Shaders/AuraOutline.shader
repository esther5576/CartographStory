...
#pragma multi_compile_fog
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		float4 texturedRim = saturate(rim.a * _Offset); // make a harder edge
		float4 extraRim = (saturate((_Edge + rim.a) * _Offset) - texturedRim) * _Brightness ;// extra edge, subtracting the textured rim
		float4 result = (_Color2 * texturedRim) + (_ColorR * extraRim);
		UNITY_APPLY_FOG(i.fogCoord, result);
		return result;// combine both with colors
	}
	ENDCG
	}
	}
